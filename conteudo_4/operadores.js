document.addEventListener("DOMContentLoaded", () => {
  ...
  const btDesafio = document.getElementsByClassName("desafio")[0];

  if (btDesafio) {
      btDesafio.addEventListener("click", () => {
          console.log("Desafio clicked");
          const perguntas = [
              {
          ...
          function atualizarQuiz() {
              ...
              if (quizPergunta) {
                  quizPergunta.innerHTML = `Pergunta ${perguntaAtual + 1}: ${perguntas[perguntaAtual].pergunta}`;
              }
          ...
          document.querySelector("#next-quiz").addEventListener("click", () => {
              if (alternativaSelecionada && alternativaSelecionada.textContent === perguntas[perguntaAtual].correta) {
                  acertos++;
              } else {
          ...
          document.querySelector(".quiz-alternativas").addEventListener("click", (e) => {
              if (e.target.classList.contains("alternativa")) {
                  let alternativaClicada = e.target;
                  if (ultimaAlternativaClicada && ultimaAlternativaClicada !== alternativaClicada) {
          ...
          function mostrarErros() {
              ...
              if (perguntasErradas.length > 0) {
                  ...
                  perguntasErradas.forEach((pergunta, index) => {
                      ...
                      erroDiv.innerHTML = `<p>Pergunta ${perguntasErradas[index] + 1}</p>`;
                      errosContainer.appendChild(erroDiv);
                  });
              } else {
                  errosContainer.innerHTML = "<p>Parabéns! Você acertou todas as perguntas!</p>";
              }
          }
